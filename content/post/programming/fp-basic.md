...
函数式编程可以看做声明式编程的一个子集。命令式编程语言越来越多地支持声明式（函数式）的编程特性。



{{% notice tip 函数式特性 %}}

支持函数式特性目前已成为主流编程语言的标配。

{{% /notice %}}



函数式编程的核心思想是：不变、无副作用、无状态。各种概念都是建立在这个基础上的，概念之间环环相扣、互相之间有着深刻的联系。函数式编程认为，一个函数的返回值是表达计算结果的唯一形式（而不是把结果写入参数或借助副作用输出）。而实际上计算机最终必须通过 IO 与人交互。 正因如此，一些函数式编程语言做了很多工作来处理 IO，由此也产生了不少难题。



函数式编程思想不使用显式的选择和循环，而选择和循环是结构化编程的两个核心。
...
map, filter, reduce 可以说是函数式编程的三板斧。很多人初学函数式编程最先接触的便是这三个函数。在响应式编程也会经常见到这几个函数。
...
```javascript
const a = [1, 2, 3, 4];constet r1 = a.map(e => e * e);
console.log(r1); // [1, 4, 9, 16]

const r2 = r1.filter(e => e % 2 == 0);
console.log(r2); // [4, 16]
```
...
```javascript
const a = [1, 2, 3, 4];
const r3 = a.reduce((s, e) => s.concat(e), "");
console.log(r3); // 1234
```
