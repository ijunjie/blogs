...
本篇是《浅谈函数式编程》的上篇，主要讲述编程范式，作为入门函数式编程必要的背景知识；下篇按照由浅入深的顺序介绍函数式编程的一些基本的概念。

函数式编程中的很多概念比较抽象，不易理解；部分概念还会涉及到一些数学理论和 PL 领域的知识。《浅谈函数式编程》将遵循实用的原则，以代码示例对一些常见概念做简明、通俗的解释，使大家对函数式编程有正确的、基本的认识。
